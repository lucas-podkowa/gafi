...
class ZKLibrary
{
    ...
    public $timeout_usec = 500000;//5000000;
    ...
    {
    	//Evaldo. Nuevas constantes agregadas
	...
	> packet(id=CMD_FREE_DATA)
    	...
    	$template_size = $data[0];
    	//echo $template_size . ' ';
    	
    	//echo 'CMD_FREE_DATA<br>>';
    	$command = CMD_FREE_DATA;
    	if($this->execCommand($command))
	    	//echo ' OK <br> ';
    	
    	//echo 'CMD_REFRESHDATA<br>>';
	...
	*/
	...
	//echo 'CMD_PREPARE_DATA <br>>';

        //$command_string = chr($template_size % 256) . chr(round($template_size / 256)) . pack("a*", "00");;
        $command_string = chr($template_size % 256) . chr(round($template_size / 256)) . chr(0) . chr(0);
        //$command_string = str_pad(pack('v*', $template_size), 2, chr(0)) . chr(0) . chr(0) ;
        //$command_string = chr($template_size % 256) . chr(round($template_size / 256)) . chr(0) . chr(0) . chr(0);
    	$command = CMD_PREPARE_DATA;
     	$this->execCommand($command, $command_string);

    	//echo 'CMD_DATA<br>>';
    	$command_string = $data[4];
    	$command = CMD_DATA;
    	$this->execCommand($command, $command_string);
   	
   	
    	//echo 'CMD_CHECKSUM_BUFFER<br>>';
    	//$command_string = chr(0);
    	$command = CMD_CHECKSUM_BUFFER;
    	$this->execCommand($command);
   
    	//echo 'CMD_TMP_WRITE<br>>';
    	$uid = $data[1];
    	$byte1 = chr((int) ($uid % 256));
        $byte2 = chr((int) ($uid >> 8));

        //echo 'id huella: ' . $data[2] . '<br>';
	$command_string = $byte1 . $byte2 . chr($data[2]) . chr(1) . chr($template_size % 256) . chr(round($template_size / 256));
    	$command = CMD_TMP_WRITE;
    	$r = $this->execCommand($command, $command_string);
    	//echo hexdec($r);
	...
	/* codigo original
		echo 'CMD_USERTEMP_WRQ<br>>';
		$command = CMD_USERTEMP_WRQ;
		$command_string = $data[4];
		if($this->execCommand($command, $command_string))
		    	echo ' OK <br> ';
	*/
    ...
    public function startEnroll($uid, $finger)
    {
       ...
       /* $byte1 = chr((int) ($uid % 256));
        ...
        //Evaldo. Corregido para que funcione con nuestros relojes
       $command_string = str_pad(pack('a*', $uid), 24, chr(0)) . pack('i*', $finger);
        ...
        try {
            ...
            if (count($this->attendance_data) > 0) {
                ...
                while (strlen($attendance_data) > 40) {
                    ...
                    // modificado por Evaldo para que lea el Estado desde nuestro modelo de terminal 
